...
const popularCurrencies = ['EUR', 'GBP', 'JPY', 'CAD', 'AUD', 'CHF', 'INR', 'SGD', 'CNY'];
...
swapBtn.addEventListener('click', function() {
  ...
  if (!fromCurrency || !toCurrency) {
    showError('Please select both currencies to swap');
    return;
  }
  ...
  const fromCurrencyObj = currencyData[fromCurrency];
  const toCurrencyObj = currencyData[toCurrency];
  
  fromDropdown.querySelector('.dropdown-text').textContent = `${fromCurrencyObj.name} (${fromCurrency})`;
  toDropdown.querySelector('.dropdown-text').textContent = `${toCurrencyObj.name} (${toCurrency})`;
...
function updateCurrencyDisplay(type, code) {
  ...
  display.querySelector('.currency-code').textContent = code;
  display.querySelector('.currency-name').textContent = currency.name;
  display.querySelector('.currency-symbol').textContent = currency.symbol;
}
